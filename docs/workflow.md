...
The `finalize` artifact is reached via `/opsx:continue` after verify reports PASS. As of schema v4, its instruction executes **the git-side closeout** directly — merging the implementation worktree branch back into the user's feature branch, pushing the feature branch (which updates the existing PR if one was opened manually for spec pre-review, or creates a remote tracking branch if not). If no PR exists at this point, finalize prompts the user to open one (with a suggested conventional-commit title), so the decision happens at the natural SDLC moment. Finalize then writes `finalize.md` and posts a single code-reviewer onboarding comment on the PR. `superpowers:finishing-a-development-branch` is retained only as a manual escape hatch for workflows that don't match the git-side closeout (solo / no-PR, brand-new PR, keep-as-is, discard). The recommended retrospective lives here as well, before archive.
