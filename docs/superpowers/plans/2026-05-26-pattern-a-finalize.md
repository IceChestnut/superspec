...
6. **Manual integration tests (Task 14)** — end-to-end verification of Pattern A and the escape hatch.
...
| `openspec/schemas/superspec/README.md` | Modify | Task 5 | Differences table finalize row, new Decision Log entry "Why we own Pattern A's logic (v4)", schema version reference |
| `docs/workflow.md` | Modify | Task 6 | Phase 5 summary subsection rewrite, schema version reference |
| `docs/workflow-details.md` | Modify | Task 7 | Phase 5 full rewrite (Pattern A canonical + escape hatch + PR comment subroutine + borrowed-logic subsection), Phase 6 golden-path rewrite, Superpowers skill index touch-point row 7 update |
| `docs/workflow-mermaid.md` | Modify | Task 8 | Phase 5 mermaid block content (Pattern A semantics instead of "finishing-a-development-branch") |
    ...
    0. **Pre-flight — commit change artifacts to feature branch**:
       ...
       d. **Branch-state checks** (v4):
          - **Recommended**: a feature branch (any branch that is not
            ...
            `master`). This is the canonical Pattern A starting state.
            Finalize's Pattern A flow will merge the worktree back
            into this branch and push to update the existing PR.
          - **Tolerated with warning**: on the integration branch.
            Proceed, but warn the user that (1) artifact commits will
            land on the integration branch and (2) finalize's Pattern A
            flow will not produce a usable PR because there is no
            separate feature branch to merge into; the user will need
            to fall back to the manual escape hatch in finalize.
          - **Detached HEAD**: warn the user that finalize's Pattern A
            flow will be skipped (no branch to merge into), and the
            user will need to use the escape hatch. Proceed with apply
            on the detached HEAD; the worktree will still be created.
```
...
**Spec reference:** "Pattern A as the canonical schema-executed flow", "Code-reviewer onboarding comment", "Escape hatch: manual skill invocation", "Schema changes (diff sketch)" sections of the design doc. The instruction body below is the assembled result of those sections.
...
```
  ...
  v4: finalize's instruction is rewritten to execute Pattern A
  directly (merge the implementation worktree branch back into the
  user's feature branch locally, push the feature branch to update
  the existing PR) rather than invoking
  superpowers:finishing-a-development-branch. The skill is retained
  as a manual escape hatch for non-Pattern-A flows. Apply step 0
...
- [ ] **Step 4: Replace the finalize artifact's `instruction:` body with the v4 Pattern A instruction.**
...
```
      Pattern A is Superspec's canonical finalize flow: merge the
      implementation worktree branch back into your feature branch
      locally, push the feature branch to update the existing PR
      (the one opened manually between plan and apply for spec
      pre-review), write the finalize.md receipt, and post a
      code-reviewer onboarding comment on the PR.

      Prerequisites for Pattern A:
      ...
      - The feature branch is NOT the integration branch (main /
        master); if it is, skip Pattern A and use the escape hatch
      ...
      reviewer. It is invoked from Pattern A (step 11) AND from the
      escape hatch (step 3 of the escape-hatch instructions below)
      ...
      If your workflow does not match Pattern A — solo / local-only
      (skill Option 1 merge-locally to main); brand-new PR via the
      skill (Option 2); keep the worktree alive for iteration
...
```bash
git add openspec/schemas/superspec/schema.yaml
git commit -m "feat(schema): rewrite finalize to execute Pattern A; bump v3→v4"
```
...
- [ ] **Step 4: Update Section 5 (Archive) to point at the new Pattern A canonical golden path.**
...
git commit -m "docs(integration): document v4 Pattern A finalize and v3→v4 migration"
```
...
- [ ] **Step 3: Add a new Decision Log entry: "Why we own Pattern A's logic (v4)".**
...
git commit -m "docs(schema): README documents v4 Pattern A decision and finalize fallback"
```
...
The `finalize` artifact is reached via `/opsx:continue` after verify reports PASS. As of schema v4, its instruction executes **Pattern A** directly — merging the implementation worktree branch back into the user's feature branch, pushing the feature branch to update the PR that was opened manually for spec pre-review, writing `finalize.md`, and posting a single code-reviewer onboarding comment on the PR. `superpowers:finishing-a-development-branch` is retained only as a manual escape hatch for workflows that don't match Pattern A (solo / no-PR, brand-new PR, keep-as-is, discard). The recommended retrospective lives here as well, before archive.
...
git commit -m "docs(workflow): Phase 5 summary reflects v4 Pattern A finalize"
```
...
git commit -m "docs(workflow-details): rewrite Phase 5 for v4 Pattern A; add borrowed-logic subsection"
```
...
git commit -m "docs(workflow-mermaid): Phase 5 reflects v4 Pattern A with escape-hatch fallback"
```
...
- Phase 5 now shows the Pattern A node (`Merge worktree → feature branch · push to update PR · post code-reviewer onboarding comment`) and the escape-hatch sub-node.
...
git commit -m "feat(docs): regenerate SVG flowchart for v4 (Pattern A finalize)"
```
...
- [ ] **Step 2: Pattern A end-to-end test.**
...
5. `/opsx:continue` → finalize executes Pattern A.
...
Prepare a test change with a deliberately long, verbose `proposal.md` and `retrospective.md`. Run Pattern A. Inspect the resulting PR comment:
...
In a test change, temporarily revoke `gh` auth (`gh auth logout`). Run Pattern A. Confirm:
...
**Files:** None modified directly; this is the recursive finalize step where v4's own implementation dogfoods Pattern A.

- [ ] **Step 1: Push the feature branch and open the PR (if you haven't already).**

```bash
git push -u origin spec/pattern-a-finalize
gh pr create --title "feat(schema): v4 Pattern A finalize rewrite" --body "$(cat <<'EOF'
## Summary
- Schema v4 promotes Pattern A to a schema-executed canonical finalize flow.
...
The Pattern A automation in the schema only kicks in for changes managed under `openspec/changes/<name>/`. Since this is a schema-level rewrite that lives under `docs/superpowers/`, it ships as a normal PR following the conventions in this repo's other recent changes (e.g., the v3 PR series).

- [ ] **Step 3: Manually post the code-reviewer onboarding comment on this PR.**

Even though Pattern A isn't auto-invoked for schema-level PRs, the comment is valuable. Write a comment that follows the same template structure (manually filled). Use `gh pr comment` to post it. This serves as a real-world dogfood test of the comment template's readability.
...
- Pattern A definition: implemented in Task 2 (finalize instruction) and documented in Tasks 4, 6, 7. ✓
