...
- [x] 1.1 创建 `package.json` — 包名 `superspec-init`，bin 入口，files 白名单
- [x] 1.2 创建 `.npmignore` — 排除 docs/、.git/ 等

## 2. CLI 核心安装流程

- [x] 2.1 实现 `--help` / `-h` / 无参数 → 显示 banner + usage
- [x] 2.2 实现 Step 1：`openspec init --profile custom`（有 `--tools <h>` 时追加 `--tools <h>`；无则进入 openspec TUI）
- [x] 2.3 实现 Step 2：将包内嵌 schema 复制到 `<cwd>/openspec/schemas/superspec/`
- [x] 2.4 实现 Step 3：写入 `openspec/config.yaml`（`schema: superspec`），已存在时区分已设置 / 不同 schema 两种情况
- [x] 2.5 实现 Step 4：运行 `openspec schemas` + `openspec validate --all`，打印成功摘要
...
- [x] 4.1 本地测试：在临时 git 仓库中运行 `node cli.js --tools opencode`，确认全流程通过
- [x] 4.2 本地测试：运行 `node cli.js`（无 `--tools`），确认进入 openspec TUI
- [x] 4.3 本地测试：模拟全局配置缺失场景，确认自动修复交互正常
- [x] 4.4 运行 `npm pack --dry-run`，确认包内容正确
- [ ] 4.5 `npm publish` 发布到 npm registry
